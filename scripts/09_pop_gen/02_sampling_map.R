...
palette_df <- read.table("scripts/09_pop_gen/palete_ancestral.tsv", header = FALSE, comment.char = "")
cluster_colors <- palette_df$V1 
names(cluster_colors) <- paste0("K", 1:6)

point_palette_df <- read.table("scripts/09_pop_gen/palette_pop.tsv", header = FALSE, comment.char = "")
